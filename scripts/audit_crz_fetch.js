#!/usr/bin/env node
// Audit DV-CRZ-SUMS generic: fetch CRZ detail pages for batch <N>, parse max amount, compare to DB.
const fs = require('fs');
const path = require('path');

const N = process.argv[2];
if (!N) { console.error('usage: node audit_crz_fetch.js <batchStart>'); process.exit(1); }
const BATCH = path.join(__dirname, '..', '.audit', `DV-CRZ-SUMS_batch_${N}.json`);
const OUT = path.join(__dirname, '..', '.audit', `DV-CRZ-SUMS_batch_${N}_result.json`);
