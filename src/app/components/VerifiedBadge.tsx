...
export default function VerifiedBadge({ source, date }: { source: string, date?: string }) {
  ...
  return (
    <div className="group relative inline-flex items-center justify-center cursor-help ml-1">
      <ShieldCheck className={`w-4 h-4 transition-colors ${jeStatny ? "text-emerald-500 hover:text-emerald-600" : "text-slate-400 hover:text-slate-500"}`} role="img" aria-label={nadpis} />
