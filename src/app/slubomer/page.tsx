...
export default async function SlubomerPage() {
  ...
  const getStatusUI = (status: string | undefined) => {
    switch(status) {
      ...
      default: return { icon: null, bg: 'bg-elevated text-body border-line' };
    }
  };
        ...
        <div className="bg-card rounded-2xl shadow-sm border border-line overflow-hidden mb-8">
          ...
          <div className="divide-y divide-line">
            {!promises || promises.length === 0 ? (
              ...
              return (
                <div key={promise.id} className="p-6 hover:bg-surface transition-colors">
                  <div className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-3">
                    <div>
                      <span className="text-xs font-bold uppercase tracking-wider text-muted mb-1 block">
                        Politik: {promise.politician_name}
                      </span>
                      <h3 className="text-xl font-bold text-body">{promise.title}</h3>
                    </div>
                  ...
                  <p className="text-muted mb-4">{promise.description}</p>
                  ...
                  {txs.length > 0 && (
                    <div className="mt-4 bg-surface rounded-xl border border-line p-4">
                      <h4 className="text-xs font-bold uppercase tracking-wider text-muted mb-3 flex items-center gap-1">
                        <FileText className="w-4 h-4" aria-hidden="true" /> Dôkazy z CRZ / Faktúr
                      </h4>
                      <div className="space-y-2">
                        {txs.map((tx: TxRow) => (
                          <div key={tx.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 p-3 bg-card rounded-lg border border-line shadow-sm">
                            <span className="text-sm font-medium text-body line-clamp-1 flex-1" title={tx.subject}>
                              {tx.subject}
                            </span>
