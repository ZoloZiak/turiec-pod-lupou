// READ-ONLY batch verify: items 40-79 of .audit/DV-ICO-ALL_set.json
const fs = require('fs');
const path = '/Users/ziak.z/projects/turiec-pod-lupou/.audit/DV-ICO-ALL_set.json';
const FROM = parseInt(process.argv[2] || '40', 10), N = 40, THROTTLE = 150;

const norm = s => (s || '')
  .toLowerCase()
  .normalize('NFD')
  .replace(/[\u0300-\u036f]/g, '')
  .replace(/\b(a\.?\s?s\.?|s\.?\s?r\.?o\.?|v\.?\s?o\.?s\.?|n\.?\s?o\.?|z\.?\s?p\.?|p\.?\s?r\.?|k\.?\s?s\.?|f\.?\s?a\.?|š\.?\s?p\.?)\b/g, ' ')
  .replace(/[^a-z0-9]+/g, ' ')
  .trim();
...
(async () => {
  const items = JSON.parse(fs.readFileSync(path, 'utf8')).items.slice(FROM, FROM + N);
  const results = [];
  for (const it of items) {
    ...
    // RÚZ API WAF-blokované → fallback ORSR web
    try {
    ...
    if (!r.status) {
      ...
      r.status = (okRpo && okRuz) ? 'OK' : (okRpo || okRuz ? 'MISMATCH' : 'MISMATCH');
    }
    results.push(r);
    console.error(`${results.length}/${N} ${it.ico} ${r.status}`);
    await sleep(THROTTLE);
  }
  fs.writeFileSync(`/tmp/dv_ico_batch_${FROM}_${FROM + N}.json`, JSON.stringify(results, null, 2));
  const counts = {};
